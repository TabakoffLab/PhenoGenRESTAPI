...
def lambda_handler(event, context):
    logger=SharedUtilityFunctions.getHelpMain()
    ...
    if (payload != None and payload != ""):
        ...
        if ('gene' in payload):
            ...
            if (geneID.startswith("PRN") or (
                ...
                if (dataType == "seq"):
                    return SharedUtilityFunctions.respond(SharedUtilityFunctions.InputError('gene',
        ...
        if ('version' in payload):
            ...
            if (geneID.find("RN6.") > 0 or geneID.find("RN7.")>0):
                tmp = geneID[geneID.find(".") + 1:][0]
                print("version:" + tmp)
                tmpversion = str(int(tmp) - 1)
                if (version != tmpversion):
                    return SharedUtilityFunctions.respond(SharedUtilityFunctions.InputError('version/geneID',
        ...
        if (cutoff > 8):
            message += " Cutoff is likely too high to return results.  Valid range: 1.0-8.0"
        elif (cutoff < 1):
            message += " Cutoff is too low to return additional results. Valid range: 1.0-8.0"
